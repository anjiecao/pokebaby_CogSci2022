...
```{r}
b_res_summary %>% 
  ggplot(aes(x = trial_number, y = log_trial_looking_time, color = complexity)) + 
  ...
  ylab("Looking time per trial (log)") + 
  xlab("Trial number") + 
  theme_classic() + 
  labs(title = "adult data")
```
...
```{r}
sim_b_res %>% 
  group_by(params_info, sequence_scheme_print) %>% 
  summarise(
  ...
  left_join(sim_b_res, by = c("params_info", "sequence_scheme_print")) %>% 
