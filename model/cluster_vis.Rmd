...
nn_sim_res <- readRDS(here("sim_res/no_noise_sim.RDS"))
random_sim_res <- readRDS(here("sim_res/random_looking_sim.RDS"))
...
nn_res_print <- tidy_up_raw_simulation_results(nn_sim_res, im_type = "EIG_nn")
r_res_print <- tidy_up_raw_simulation_results(random_sim_res, im_type = "random")
...
nn_res_sum <- summarise_tidy_sim_res(nn_res_print)
r_res_sum <- summarise_tidy_sim_res(r_res_print)
...
nn_sim_b_res <- nn_res_sum %>% 
   left_join(b_res_summary, by = c("trial_number", "sequence_scheme", "sequence_scheme_print", "complexity"))

r_sim_b_res <- r_res_sum %>% 
   left_join(b_res_summary, by = c("trial_number", "sequence_scheme", "sequence_scheme_print", "complexity"))
```
...
df_nn_r <- calculate_correlation(nn_sim_b_res)
df_r_r <- calculate_correlation(r_sim_b_res)
...
scaled_nn_res <- scale_model_results(b_res_print, nn_res_print, best_params = "ae_1_be_10_ap_1_bp_4_np_0_wEIG_0.01")
scaled_r_res <- scale_model_results(b_res_print, r_res_print, best_params = NA)
...
saveRDS(scaled_r_res, here("scaled_random_model.RDS"))
saveRDS(scaled_nn_res, here("scaled_no_noise_res.RDS"))
```
